...
exports.apiKey = "AIzaSyAbPTwwqzZx_RZtHvsgc6mz6IPVH0P_2Nk";
...
Map.prototype.playGame = function() {
  ...
  if (navigator.geolocation) {
      ...
      //see if the answer was correct by checking longitude and latitude
      _this.map.addListener("click", function (event) {
        ...
        $.get('https://maps.googleapis.com/maps/api/geocode/json?latlng=' + latitude + ',' + longitude + '&key=' + apiKey).then(function(response) {
          ...
          if (_this.answer.toLowerCase() === _this.userGuess.toLowerCase()) {
            ...
            $('#instruction').text("Sorry, game over! You clicked on " + _this.userGuess + ". See the map for the location of the correct answer. Want to play again?");
            $('#next').show();
            //add reset game button that clears the board and score
            $('#country').hide();
